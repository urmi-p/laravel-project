...
class EncodeVideoVault implements ShouldQueue
{
   ...
   * @return void
   */
  public function __construct(public Vault $video) {}
  ...
  public function handle()
  {
    ...
    try {
      ...
      Vault::whereId($this->video->id)->update([
      ...
      Notifications::send($this->video->user_id, $this->video->user_id, 35, $this->video->id, $this->video->file_name);
    } catch (\Exception $e) {
      // Delete file
  ...
  {
    // Log the error
    \Log::error('Video encoding Vault failed', [
    ...
    Notifications::send($this->video->user_id, $this->video->user_id, 36, $this->video->id, $this->video->file_name);
