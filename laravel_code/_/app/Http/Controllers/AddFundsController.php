...
class AddFundsController extends Controller
{
  ...
  public function __construct(Request $request, AdminSettings $settings, private OpenPixClient $openpix)
  {
  ...
  public function send()
  {
    ...
    switch ($this->request->payment_gateway) {
      ...
      case 'OpenPix':
        return $this->sendOpenPix();
        break;

      case 'Atlos':
        return $this->sendAtlos();
        break;
  ...
  public function mercadoPagoProcess(Request $request)
  {
    try {
      ...
      if ($paymentData->status !== 'approved') {
        return redirect('my/wallet')->withErrorMessage(__('general.payment_cancelled'));
      }
  ...
  public function sendBinance()
  {
    ...
    $request = [
      ...
      "goods" => [
        ...
        "goodsName" => __('general.add_funds')
      ],
      'returnUrl' => url('my/wallet'),
      'cancelUrl' => url('my/wallet'),
      'webhookUrl' => route('webhook.binance', ['data' => $data])
    ];
        ...
        switch ($data['type']) {
          case 'deposit':
            ...
            if (!isset($verifiedTxnId)) {
              ...
              info('Error Binance (returnCode) - ' . $response['returnMessage']);
            }
            break;
        }
      }
  ...
  public function sendOpenPix()
  {
    try {
      // Get Payment Gateway
      $payment = PaymentGateways::whereName('OpenPix')->firstOrFail();
      $fee   = $payment->fee;
      $cents = $payment->fee_cents;
      $taxes = config('settings.tax_on_wallet') ? ($this->request->amount * auth()->user()->isTaxable()->sum('percentage') / 100) : 0;
      $amount = number_format($this->request->amount + ($this->request->amount * $fee / 100) + $cents + $taxes, 2, '.', '');
      ...
      $deposit->txn_id = $orderId;
      $deposit->amount = $this->request->amount;
      $deposit->payment_gateway = 'OpenPix';
      ...
      $reponse = $this->openpix->charges()->create([
        'correlationID' => $orderId,
        'value' => ($amount * 100)
      ]);

      if (isset($reponse['errors'])) {
        Deposits::whereTxnId($orderId)->whereStatus('initialized')->delete();

        return response()->json([
          'success' => false,
          'errors' => ['error' => $reponse['errors'][0]['message']],
        ]);
      }

      return response()->json([
        'success' => true,
        'url' => $reponse['charge']['paymentLinkUrl'],
      ]);
    } catch (\Exception $e) {
      Deposits::whereTxnId($orderId)->whereStatus('initialized')->delete();
  ...
  public function sendAtlos()
  {
    ...
    $urlReturn = url('my/wallet');
    $urlWebhook = route('webhook.atlos');
