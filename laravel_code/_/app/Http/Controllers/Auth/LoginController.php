...
class LoginController extends Controller
{
  ...
  public function login(Request $request)
  {
    ...
    $messages = [
      'g-recaptcha-response.required_if' => __('admin.captcha_error_required'),
    ...
    if ($this->auth->attempt($credentials, $request->has('remember'))) {
      ...
      if ($this->auth->user()->status == 'active' || $this->auth->user()->status == 'disabled') {
        ...
        return response()->json([
          'success' => false,
          'errors' => ['error' => __('validation.user_suspended')],
        ]);
        ...
        return response()->json([
          'success' => false,
          'errors' => ['error' => __('validation.account_not_confirmed')],
        ]);
      }
    }
