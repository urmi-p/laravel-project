...

{
	...
	public static function reelPlaybackUrl($media): string
	{
		...
		if (!empty($media->bunny_video_id) && env('BUNNY_STREAM_CDN_HOSTNAME')) {
			$mode = strtolower((string) env('BUNNY_STREAM_PLAYBACK_MODE', 'mp4'));
			$cdnHostname = env('BUNNY_STREAM_CDN_HOSTNAME');
			if ($mode === 'original') {
				return 'https://' . $cdnHostname . '/' . $media->bunny_video_id . '/original';
			}
			if ($mode === 'hls') {
				return 'https://' . $cdnHostname . '/' . $media->bunny_video_id . '/playlist.m3u8';
			}
			if ($mode === 'direct') {
				return 'https://video.bunnycdn.com/play/' . env('BUNNY_STREAM_LIBRARY_ID') . '/' . $media->bunny_video_id;
			}
			$playbackFile = env('BUNNY_STREAM_PLAYBACK_FILE', 'play_720p.mp4');
			return 'https://' . $cdnHostname . '/' . $media->bunny_video_id . '/' . $playbackFile;
		}
		return !empty($media->name) ? self::getFile(config('path.reels') . $media->name) : '';
	}
	public static function reelThumbnailUrl($media, string $fallback = ''): string
	{
		if (!$media) {
			return $fallback;
		}
		if (!empty($media->bunny_video_id) && env('BUNNY_STREAM_CDN_HOSTNAME')) {
			return 'https://' . env('BUNNY_STREAM_CDN_HOSTNAME') . '/' . $media->bunny_video_id . '/thumbnail.jpg';
		}
		if (!empty($media->video_poster) && filter_var($media->video_poster, FILTER_VALIDATE_URL)) {
			return $media->video_poster;
		}
		if (!empty($media->video_poster)) {
			return self::getFile(config('path.reels') . $media->video_poster);
		}
		return $fallback;
	}

	public static function storyPlaybackUrl($media): string
	{
		if (!$media) {
			return '';
		}
		...
		if ($media->type === 'photo') {
			return self::storyFileUrl($media->name ?? '');
		}
	...
	public static function storyThumbnailUrl($media, string $fallback = ''): string
	{
		if (!$media) {
			return $fallback;
		}
		...
		if ($media->type === 'photo') {
			return self::storyFileUrl($media->name ?? '') ?: $fallback;
		}
	...
	public static function storyFileUrl($name): string
	{
		if (empty($name)) {
			return '';
		}
	...
	public static function normalizedCdnBaseUrl(string $url): string
	{
		$url = trim($url);
		if ($url === '') {
			return '';
	...
	public static function vaultPlaybackUrl($media): string
	{
		if (!$media) {
			return '';
		}
		...
		return !empty($media->file) ? self::vaultFileUrl($media->file) : '';
	}

	public static function vaultThumbnailUrl($media, string $fallback = ''): string
	{
		if (!$media) {
			return $fallback;
		}
	...
	public static function vaultFileUrl($name): string
	{
		if (empty($name)) {
			return '';
		}
	...
	public static function messagePlaybackUrl($media): string
	{
		if (!$media) {
			return '';
		}
		...
		if (!empty($media->file) && $media->vault_id) {
			return self::vaultFileUrl($media->file);
		}

		$path = $media->vault_id ? config('path.vault') : config('path.messages');
		return !empty($media->file) ? self::getFile($path . $media->file) : '';
	}

	public static function messageImageUrl($media, $messageId = null): string
	{
		if (!$media || empty($media->file)) {
			return '';
		}

		if (!empty($media->vault_id)) {
			return self::vaultFileUrl($media->file);
		}
		...
		if (!$isLocal && env('BUNNY_PULL_ZONE_URL')) {
			return self::normalizedCdnBaseUrl(env('BUNNY_PULL_ZONE_URL')) . '/' . ltrim($path, '/');
		}
	...
	public static function messageThumbnailUrl($media, string $fallback = ''): string
	{
		if (!$media) {
			return $fallback;
		}
	...
	public static function postImageUrl($media): string
	{
	...
	public static function shopFileUrl($name): string
	{
		if (empty($name)) {
			return '';
		}
	...
	public static function adsFileUrl($name): string
	{
		if (empty($name)) {
			return '';
		}
	...
	public static function giftImageUrl($name): string
	{
		if (empty($name)) {
			return '';
		}
		...
		$relativePath = 'img/gifts/' . ltrim($name, '/');
		$publicPath = public_path($relativePath);

		if (!file_exists($publicPath) && env('BUNNY_PULL_ZONE_URL')) {
			return self::normalizedCdnBaseUrl(env('BUNNY_PULL_ZONE_URL')) . '/' . ltrim($relativePath, '/');
		}
	...
	public static function postPlaybackUrl($media): string
	{
		if (!$media) {
			return '';
		}
		...
		return !empty($media->video) ? self::getFile(config('path.videos') . $media->video) : '';
	}

	public static function postThumbnailUrl($media, string $fallback = ''): string
	{
		if (!$media) {
			return $fallback;
		}
		...
		if (!empty($media->video_poster)) {
			return self::getFile(config('path.videos') . $media->video_poster);
		}
		return $fallback;
	}

	public static function welcomeMessagePlaybackUrl($media): string
	{
		if (!$media) {
			return '';
		}
		if (!empty($media->bunny_video_id) && env('BUNNY_STREAM_CDN_HOSTNAME')) {
			...
			$playbackFile = env('BUNNY_STREAM_PLAYBACK_FILE', 'play_240p.mp4');
			return 'https://' . $cdnHostname . '/' . $media->bunny_video_id . '/' . $playbackFile;
		}
		return !empty($media->file) ? self::welcomeMessageFileUrl($media->file) : '';
	}

	public static function welcomeMessageThumbnailUrl($media, string $fallback = ''): string
	{
		if (!$media) {
			return $fallback;
		}
	...
	public static function welcomeMessageFileUrl($name): string
	{
		if (empty($name)) {
			return '';
		}
