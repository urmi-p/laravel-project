    ...
    <div id="app" class="h-full md:flex md:justify-center md:items-center">
        <div class="w-full max-w-lg">
            <h1 class="text-4xl font-bold text-center p-4 sm:p-6 mt-4">
                {{ trans('general.your_amount_payment', ['amount' => $amount]) }}
            </h1>
            ...
            <div class="bg-white rounded-lg shadow-xl p-4 sm:p-6 mt-4">
                <div v-if="paymentIntent.status === 'succeeded'">
                    <h2 class="text-xl mb-4 text-gray-600">
                        {{ trans('general.payment_successful') }}
                    </h2>

                    <p class="mb-6">
                        {{ trans('general.payment_already_successfully_confirmed') }}
                    </p>
                </div>

                <div v-else-if="paymentIntent.status === 'processing'">
                    <h2 class="text-xl mb-4 text-gray-600">
                        {{ trans('general.payment_processing') }}
                    </h2>

                    <p class="mb-6">
                      {{ trans('general.payment_processing_info') }}
                    </p>
                </div>

                <div v-else-if="paymentIntent.status === 'canceled'">
                    <h2 class="text-xl mb-4 text-gray-600">
                        {{ trans('general.payment_cancelled') }}
                    </h2>

                    <p class="mb-6">
                        {{ trans('general.payment_was_cancelled') }}
                    </p>
                </div>
                ...
                <div v-else>
                    ...
                    <div v-if="paymentIntent.status === 'requires_payment_method'" class="mb-3">
                        ...
                        <p class="mb-6">
                            {{ trans('general.stripe_payment_info') }}
                        </p>
                        ...
                        <div v-else>
                            <p class="text-sm mb-3">
                                {{ trans('general.stripe_text_info_1') }} @{{ paymentMethodTitle }}.
                            </p>
                        </div>
                        ...
                        <div v-if="paymentElement">
                            ...
                            <label for="payment-element" class="inline-block text-sm text-gray-700 font-semibold mb-2">
                                {{ trans('general.stripe_text_info_2') }}
                            </label>
                        ...
                        <div v-if="(paymentMethod || {}).remember">
                            ...
                            <label for="remember" class="inline-block text-sm text-gray-700 mb-2">
                                ...
                                {{ trans('general.stripe_text_info_3') }}
                            </label>
                    ...
                    >
                        ...
                        <span v-else>
                          {{ trans('general.stripe_text_info_4', ['amount' => $amount]) }} @{{ paymentMethodTitle }}                        </span>
                    </button>
                </div>

                <button @click="goBack" ref="goBackButton" data-redirect="{{ $redirect ?? url()->previous() }}"
                ...
                confirmCallback: function (result) {
                    ...
                    if (result.error) {
                        if (result.error.code === '{{ Stripe\ErrorObject::CODE_PARAMETER_INVALID_EMPTY }}') {
                            this.errorMessage = '{{ trans('general.stripe_text_info_5') }}';
                        } else {
