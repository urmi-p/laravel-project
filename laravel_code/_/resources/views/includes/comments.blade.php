    ...
    <div class="media-body">
      ...
      @endif
        ...
        @if ($comment->gif_image)
          ...
          <div class="w-100 d-block mt-2"><img class="rounded" src="{{ $comment->gif_image }}" width="200"></div>
      ...
      <span class="likeComment c-pointer float-right pulse-btn" data-id="{{$comment->id}}" data-type="isComment">
        <i class="@if (auth()->check() && $comment->likes->where('user_id', auth()->id())->first()) bi bi-hand-thumbs-up-fill text-red mr-1 @else bi bi-hand-thumbs-up-fill mr-1 @endif"></i>
