...
@if ((auth()->user()->id != $response->creator->id && $checkUserSubscription && $response->price == 0.0 && $settings->disable_tips == 'off') || (auth()->user()->id != $response->creator->id && $checkUserSubscription && $response->price != 0.0 && $checkPayPerView && $settings->disable_tips == 'off') || (auth()->check() && $response->locked == 'yes' && $response->price != 0.0 && !$checkUserSubscription && $checkPayPerView && $settings->disable_tips == 'off') || (auth()->user()->id != $response->creator->id && $response->locked == 'no' && $settings->disable_tips == 'off'))
...
<div class="container-comments @if (!isset($inPostDetail)) display-none @endif">
    ...
    <div class="container-media">
        ...
        @if (auth()->user()->id == $response->creator->id || ($response->locked == 'yes' && $checkUserSubscription && $response->price == 0.0) || ($response->locked == 'yes' && $checkUserSubscription && $response->price != 0.0 && $checkPayPerView) || (auth()->check() && $response->locked == 'yes' && $response->price != 0.0 && !$checkUserSubscription && $checkPayPerView) || (auth()->user()->role == 'admin' && auth()->user()->permission == 'all') || $response->locked == 'no')
        @include('includes.comments')
        @endif
        @endif
            ...
            <form action="{{ url('comment/store') }}" method="post"
                ...
                <div class="position-absolute" class="comment-icons">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-emoji dropdown-gifs custom-scrollbar"
                            aria-labelledby="dropdownGif{{ $response->id }}"></div>
